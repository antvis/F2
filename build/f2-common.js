...
var LegendController = function () {
  ...
  LegendController.prototype.bindEvents = function bindEvents() {
    ...
    var triggerOn = legendCfg.triggerOn || 'click';
...
var TooltipController = function () {
  ...
  TooltipController.prototype._showTooltip = function _showTooltip(point) {
    var self = this;
    var chart = self.chart;
...
var Tooltip = function () {
  ...
  Tooltip.prototype.setPosition = function setPosition(items) {
    ...
    var posX = 0;
    var posY = tl.y - height - 8 + offsetY; // 垂直方向贴着图表绘图区域上方边缘
    ...
    if (fixed) {
      ...
      if (tooltipArrow) {
        tooltipArrow.attr('points', [{ x: _x - 6, y: tl.y - 8 + offsetY }, { x: _x + 6, y: tl.y - 8 + offsetY }, { x: _x, y: tl.y + offsetY }]);
        if (_x === tl.x) {
          tooltipArrow.attr('points', [{ x: tl.x, y: tl.y + offsetX }, { x: tl.x, y: tl.y - 8 + offsetY }, { x: tl.x + 8, y: tl.y - 8 + offsetY }]);
        } else if (_x === tr.x) {
          tooltipArrow.attr('points', [{ x: tr.x, y: tl.y + offsetX }, { x: tr.x - 8, y: tl.y - 8 + offsetY }, { x: tr.x, y: tl.y - 8 + offsetY }]);
        }
      }
    }
...
var F2 = __webpack_require__(40); // 核心包
F2.Plugin = __webpack_require__(67); // 插件
// 注册插件
...
//# sourceMappingURL=f2-common.js.map