...
Chart.plugins.register(Plugin.Tooltip); // 方式一：全局注册 
...
// 动态引入方式，然后使用上述方式注册
const { Tooltip } = require('@antv/f2/lib/plugin/');
```
...
chart.tooltip({
  triggerOn: [ 'touchstart', 'touchmove' ], // 出现的触发行为，可自定义，用法同 legend 的 triggerOn
...
<img src="https://gw.alipayobjects.com/zos/rmsportal/HhvpGbEFsDIsROTvLsou.gif">
